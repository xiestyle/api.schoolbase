//------------------------------------------------------------------------------
// <auto-generated>
//     This code was generated from a template.
//
//     Changes to this file may cause incorrect behavior and will be lost if
//     the code is regenerated.
// </auto-generated>
//------------------------------------------------------------------------------

using System;
using System.Data;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Collections.Specialized;

namespace api.schoolbase.entities.DBBase
{
    
    public partial class _DBBase
    {
        public static @__api.schoolbase.entities.DBBase.Entity.Base_GroupUser_Columns Base_GroupUser = new @__api.schoolbase.entities.DBBase.Entity.Base_GroupUser_Columns();
    }
    namespace @__api.schoolbase.entities.DBBase.Entity
    {
        public class Base_GroupUser_Columns : MyLib.DbAccess.SQL.IQueryTable
        {
            private const string ___TABLE_NAME = "Base_GroupUser";
            private string ___aliasName;
            public Base_GroupUser_Columns(string aliasName) : base("DBBase") { this.___aliasName = aliasName; }
            public Base_GroupUser_Columns() : base("DBBase") { this.___aliasName = ___TABLE_NAME; }
    
        #region Table Columns
    
    		private static MyLib.DbAccess.SQL.QueryColumn _GroupID = new MyLib.DbAccess.SQL.QueryColumn((___TABLE_NAME + ".GroupID"), DbType.Int64);
            public MyLib.DbAccess.SQL.QueryColumn GroupID
            {
                get
                {
                    if ((this.___aliasName == ___TABLE_NAME)) return _GroupID;
                    else return new MyLib.DbAccess.SQL.QueryColumn((this.___aliasName + ".GroupID"), DbType.Int64);
                }
            }
    
    		private static MyLib.DbAccess.SQL.QueryColumn _UID = new MyLib.DbAccess.SQL.QueryColumn((___TABLE_NAME + ".UID"), DbType.Int64);
            public MyLib.DbAccess.SQL.QueryColumn UID
            {
                get
                {
                    if ((this.___aliasName == ___TABLE_NAME)) return _UID;
                    else return new MyLib.DbAccess.SQL.QueryColumn((this.___aliasName + ".UID"), DbType.Int64);
                }
            }
    
    		private static MyLib.DbAccess.SQL.QueryColumn _Switch = new MyLib.DbAccess.SQL.QueryColumn((___TABLE_NAME + ".Switch"), DbType.Boolean);
            public MyLib.DbAccess.SQL.QueryColumn Switch
            {
                get
                {
                    if ((this.___aliasName == ___TABLE_NAME)) return _Switch;
                    else return new MyLib.DbAccess.SQL.QueryColumn((this.___aliasName + ".Switch"), DbType.Boolean);
                }
            }

        #endregion

                
            public override string ___GetTableName() { return ___TABLE_NAME; }
    		public override string ___GetAliasName() { return ___aliasName ?? ___TABLE_NAME; }
    				        public override MyLib.DbAccess.SQL.QueryColumn ___GetIdentityColumn() { return _GroupID; }
            		public @__api.schoolbase.entities.DBBase.Entity.Base_GroupUser_Columns @__Alias(string aliasName) { return new @__api.schoolbase.entities.DBBase.Entity.Base_GroupUser_Columns(aliasName); }
        }
    }
    [System.Serializable]
    public partial class Base_GroupUser
    {
        #region Property
    
        public virtual long GroupID
        {
            get { return _groupID; }
            set
            {
                if (_groupID != value)
                {
                    if (Base_Group != null && Base_Group.SERID != value)
                    {
                        Base_Group = null;
                    }
                    _groupID = value;
                }
            }
        }
        private long _groupID;
    
        public virtual long UID
        {
            get { return _uID; }
            set
            {
                if (_uID != value)
                {
                    if (tUser_ex != null && tUser_ex.UID != value)
                    {
                        tUser_ex = null;
                    }
                    _uID = value;
                }
            }
        }
        private long _uID;
    
        public virtual bool Switch
        {
            get;
            set;
        }

        #endregion

        #region Navigation Properties
    
        public virtual Base_Group Base_Group
        {
            get { return _base_Group; }
            set
            {
                if (!ReferenceEquals(_base_Group, value))
                {
                    var previousValue = _base_Group;
                    _base_Group = value;
                    FixupBase_Group(previousValue);
                }
            }
        }
        private Base_Group _base_Group;
    
        public virtual tUser_ex tUser_ex
        {
            get { return _tUser_ex; }
            set
            {
                if (!ReferenceEquals(_tUser_ex, value))
                {
                    var previousValue = _tUser_ex;
                    _tUser_ex = value;
                    FixuptUser_ex(previousValue);
                }
            }
        }
        private tUser_ex _tUser_ex;

        #endregion

        #region Association Fixup
    
        private void FixupBase_Group(Base_Group previousValue)
        {
            if (previousValue != null && previousValue.Base_GroupUser.Contains(this))
            {
                previousValue.Base_GroupUser.Remove(this);
            }
    
            if (Base_Group != null)
            {
                if (!Base_Group.Base_GroupUser.Contains(this))
                {
                    Base_Group.Base_GroupUser.Add(this);
                }
                if (GroupID != Base_Group.SERID)
                {
                    GroupID = Base_Group.SERID;
                }
            }
        }
    
        private void FixuptUser_ex(tUser_ex previousValue)
        {
            if (previousValue != null && previousValue.Base_GroupUser.Contains(this))
            {
                previousValue.Base_GroupUser.Remove(this);
            }
    
            if (tUser_ex != null)
            {
                if (!tUser_ex.Base_GroupUser.Contains(this))
                {
                    tUser_ex.Base_GroupUser.Add(this);
                }
                if (UID != tUser_ex.UID)
                {
                    UID = tUser_ex.UID;
                }
            }
        }

        #endregion

    }
}
